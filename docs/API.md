...
> `status` 取值：收到的短信为 `stored`；发送为 `queued` / `sending` / `sent` / `failed`。收到的超长短信（多段分片）会按 PDU 里的真实段序号拼接为**一条** `stored` 短信入库。

> 入库短信的 `raw` 字段：发送失败时存 AT 原始返回（如 `[GSM] >|+CMS ERROR: 331`）；收到时存分片序号与连接参数（如长短信 `idx=0,1,2 ref=77 total=3 seq=1,2,3 scts=26/09/21,19:43:55+32`，单条为 `idx=3 scts=26/09/21,19:44:01+32`）。
